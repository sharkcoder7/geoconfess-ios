...
class User: NSObject, Observable, CLLocationManagerDelegate, AppObserver {
	...
	required init(oauth: OAuthTokens, userData: JSON, subclassData: JSON?) throws {
		...
		super.init()
		self._notificationManager = NotificationManager(forUser: self)
		
		App.addObserver(self)
		initLocationTracking()
		refreshNearbySpotsCache()
	}
	...
	func updateEmail(email: String, name: String, surname: String, phoneNumber: String,
		...
		Alamofire.request(.PUT, updateURL, parameters: params).responseJSON {
			response in
	...
	func locationManager(manager: CLLocationManager,
		...
		notifyObservers { $0.user(self, didUpdateLocation: mostRecentLocation) }
	...
	private func updateNearbySpotsCacheAt(location: CLLocation) {
		...
		func scheduleNextUpdateWithTimeInterval(interval: Double) {
			spotsRefreshTimer = Timer.scheduledTimerWithTimeInterval(interval) {
				[weak self] in
		...
		let radius = User.spotsMaxRadiusInKm
	...
	private static var spotsMaxRadiusInKm: Double {
