...
final class LoginViewController: AppViewController, UITextFieldDelegate {
    ...
    @IBAction func loginButtonTapped(sender: UIButton) {
        ...
        if (email.isEmpty) {
            self.showAlert(message: "Merci d'entrer votre adresse email.")
            return
        }
        ...
        if (password.isEmpty || User.isValidPassword(password) == false)
        {
            self.showAlert(message: "Le mot de passe doit faire plus de 6 caractères.")
            return
        }
	...
	private func login(email email: String, password: String) {
		...
		User.loginInBackground(username: email, password: password) {
			...
			case .Failure(let error):
				...
				self.showAlert(message: "Email ou mot de passe incorrect.")
