...
final class FavoritePriestCell: SESlideTableViewCell {
	...
	@IBOutlet weak private var nextLabel: UILabel!
	...
	override func setSelected(selected: Bool, animated: Bool) {
		...
		if selected {
			...
			viewPriestCard.image  = UIImage(named: "next-2")
			...
			priestIcon.image = UIImage(named: "logo-couleur-liste");
			viewPriestCard.image  = UIImage(named: "next-1")
			nextLabel.textColor = FavoritePriestCell.selectionColor
