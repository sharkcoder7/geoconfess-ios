...
final class App {
	...
	static let googleMapsApiKey = "AIzaSyCVCVu4E5UpoZcCfapDrJl4H7HfBNDt74c" // Back to Paulo's.
	...
	enum Configuration: String {
		...
		var plistName: String {
			switch self {
			case .Distribution: return "GeoConfess-Distribution"
	...
	static func initConfiguration() {
		// Accessing `configuration` property forces initialization.
		log("Initial config: \(App.configuration)")
	}
	...
	private static func notifyObservers(notify: AppObserver -> Void) {
...
let GeoConfessErrorDomain = "GeoConfess"

enum ErrorCode: Int {

	case InternetConnectivityError = 1_000
	case InternalServerError = 2_000
	
	var localizedDescription: String {
		switch self {
		case .InternetConnectivityError:
			return "Aucune connexion à internet detectée."
...
extension NSError {
	...
	convenience init(code: ErrorCode) {
		...
		self.init(domain: GeoConfessErrorDomain, code: code.rawValue, userInfo: userInfo)
	}
	
	var appCode: ErrorCode? {
		guard domain == GeoConfessErrorDomain else { return nil }
